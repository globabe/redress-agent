...
import { connectWallet, disconnectWallet, getChainId, switchToStudioNetwork } from "@/lib/genlayer";
import {
  ...
  getEscrowVerdict,
  parseContractJson,
  recordEscrowPurchase,
} from "@/lib/genlayer-escrow";

export const Route = createFileRoute("/demo-escrow")({
  head: () => ({
    meta: [
      { title: "Redress Escrow Demo — Funded Intent Adjudication" },
      {
        name: "description",
        content:
          "A GenLayer demo where escrowed GEN is released to the merchant or refunded based on the verdict.",
      },
      { property: "og:title", content: "Redress — Escrowed Intent Adjudication" },
      {
        property: "og:description",
        content: "Escrow GEN, let an agent buy, and see the verdict settle the funds on-chain.",
      },
      { property: "og:type", content: "website" },
      { name: "twitter:card", content: "summary_large_image" },
    ],
  }),
  component: EscrowDemoPage,
});
...
function parseDecimalField(rawValue: string): number {
  const parsed = Number(rawValue);
...
const steps = ["Intent", "Purchase", "Adjudicate", "Settlement"];

function EscrowDemoPage() {
  const [intent, setIntent] = useState(emptyIntent);
  const [rawNumeric, setRawNumeric] = useState(emptyRaw);
  const [merchantAddress, setMerchantAddress] = useState(DEFAULT_MERCHANT_ADDRESS);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      setPurchase(createSimulatedPurchase(intent, false));
      setAgentReady(true);
    }, 1600);
    return () => window.clearTimeout(timer);
  }, [agentReady, intent, step]);
  ...
  async function handleCreateMandate() {
    ...
    if (!rawNumeric.escrowGen || intent.escrowGen <= 0) {
      setError("Enter the amount of GEN to place in escrow.");
      return;
    }
    setBusy("mandate");
    setError("");
    try {
      const returnedId = await createEscrowMandate(
        intent.product,
        specs,
        intent.maxPrice,
        intent.deadlineDays,
  ...
  async function handlePurchase() {
    ...
    if (!/^0x[a-fA-F0-9]{40}$/.test(merchantAddress.trim())) {
      return setError("Enter a valid merchant wallet address (0x… 40 hex characters).");
    }
    setBusy("purchase");
    setError("");
    try {
      await recordEscrowPurchase(
        mandateId,
        purchase.product,
        purchaseSpecs,
        purchase.price,
  ...
  async function handleAdjudicate() {
    ...
    setError("");
    try {
      await adjudicateEscrow(mandateId);
      const normalized = parseContractJson(await getEscrowVerdict(mandateId));
      ...
      setVerdict({
        verdict: result,
        remedy: String(normalized["remedy"] ?? (fulfilled ? "NONE" : "REFUND")),
        reason: String(
          normalized["reason"] ??
            (fulfilled
      ...
      let mandateData: Record<string, unknown> = {};
      try {
        mandateData = parseContractJson(await getEscrowMandate(mandateId));
      } catch {
        mandateData = {};
      }
      setSettlement(describeSettlement(result, intent.escrowGen, mandateData, normalized));
      setStep(4);
    } catch (caught) {
      showError(caught);
    } finally {
  ...
  function restart() {
    ...
    setRawNumeric(emptyRaw);
    setMerchantAddress(DEFAULT_MERCHANT_ADDRESS);
    setStep(1);
  }
      ...
      <div className="relative mx-auto max-w-5xl px-5 py-8 sm:px-8 sm:py-10">
        ...
        <main className="mt-10 grid gap-6 lg:grid-cols-12">
          <section className="glow-teal rounded-2xl border border-teal/25 bg-ink-2/85 p-6 lg:col-span-5">
            <StepHeader
              number="01"
              label="create_mandate() · payable"
              title="Fund Purchase Intent"
              tone="teal"
            />
            <p className="mt-1 text-sm text-muted-foreground">
              State what you want and lock escrowed GEN behind it.
            </p>
            ...
            <div className="mt-6 space-y-4">
              ...
              <Field label="Escrow amount (GEN)">
                <input
                  aria-label="Escrow amount (GEN)"
                  type="number"
                  step="any"
                  min="0"
                  value={rawNumeric.escrowGen}
                  onChange={(event) => {
                    const raw = event.target.value;
                    setRawNumeric({ ...rawNumeric, escrowGen: raw });
                    setIntent({ ...intent, escrowGen: parseDecimalField(raw) });
                  }}
                />
              </Field>
            </div>
            <div className="mt-5 rounded-lg border border-border/70 bg-ink/70 px-3.5 py-2.5 font-mono text-[11px] text-muted-foreground">
              ...
              <br />
              value: <span className="text-teal">{intent.escrowGen || 0} GEN</span>
            </div>
            <Button
            ...
            >
              ...
              {busy === "mandate" ? "Confirming on-chain…" : "Submit Intent & Escrow"}
            </Button>
            <p className="mt-2 text-center text-[11px] text-muted-foreground">
              The escrow amount is sent with the transaction and held by the contract.
            </p>
          </section>
          ...
          <section className="glow-violet rounded-2xl border border-violet/25 bg-ink-2/85 p-6 lg:col-span-7">
            ...
            <div className="mt-4">
              <Field label="Merchant wallet address">
                <input
                  aria-label="Merchant wallet address"
              ...
              <p className="mt-1.5 text-[11px] text-muted-foreground">
                Escrowed GEN is released to this address if the verdict is FULFILLED.
              </p>
            </div>
            <Button
          ...
          <section className="glow-coral rounded-2xl border border-coral/25 bg-ink-2/85 p-6 lg:col-span-7">
            ...
            >
              ...
              {busy === "adjudicate" ? "Validators reviewing the claim…" : "Request Redress"}
            </Button>
            {!purchaseRecorded && (
              <p className="mt-2 text-center text-[11px] text-muted-foreground">
          ...
          <section className="glow-teal rounded-2xl border border-emerald/25 bg-ink-2/85 p-6 lg:col-span-5">
            <StepHeader number="04" label="escrow settlement" title="Settlement" tone="emerald" />
            <p className="mt-1 text-sm text-muted-foreground">
              What the verdict did with the escrowed GEN.
            </p>
            {settlement ? (
              <div
                className={`mt-6 rounded-xl border p-5 ${settlement.fulfilled ? "border-emerald/40 bg-emerald/10" : "border-coral/40 bg-coral/10"}`}
              >
                <div className="flex items-center gap-3">
                  <Coins
                    className={`size-5 ${settlement.fulfilled ? "text-emerald" : "text-coral"}`}
                  />
                  <span
                    className={`rounded-lg border px-3 py-1.5 font-display text-sm font-bold tracking-wide ${settlement.fulfilled ? "border-emerald/50 bg-emerald/20 text-emerald" : "border-coral/50 bg-coral/20 text-coral"}`}
                  >
                    {settlement.fulfilled ? "RELEASED" : "REFUNDED"}
                  </span>
                </div>
                ...
                <p className="mt-3 break-all text-xs text-muted-foreground">
                  {settlement.fulfilled ? `Merchant: ${merchantAddress}` : "Returned to the buyer."}
                </p>
              </div>
            ) : (
              <div className="mt-6 grid min-h-40 place-items-center rounded-xl border border-dashed border-emerald/30 bg-ink/40 px-6 text-center">
                <p className="text-sm text-muted-foreground">
                  Adjudicate to see whether the escrow is released or refunded.
                </p>
              </div>
            )}
          </section>
        ...
        <footer className="mt-8 flex flex-col gap-2 text-xs text-muted-foreground sm:flex-row sm:items-center sm:justify-between">
          <span>
            ...
            <span className="font-mono text-foreground/70">{ESCROW_CONTRACT_ADDRESS}</span>
          </span>
